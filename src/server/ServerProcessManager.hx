...
class ServerProcessManager {
     ...
     * Counts log lines before sending, writes the command to stdin,
     * waits for Factorio to process, then returns any new log lines as response.
     * Uses factorio-current.log since Factorio redirects output there.
     */
    public function sendConsoleCommand(id:String, command:String):Null<String> {
        if (!processes.exists(id)) return null;
        try {
            var info = processes.get(id);
            var versionDir = this.factorioManager.getVersionDir(info.instance.version);
            var logPath = versionDir + "/factorio/factorio-current.log";

            // Get current line count via wc -l (avoid reading entire file)
            var beforeCount = 0;
            if (sys.FileSystem.exists(logPath)) {
                try {
                    var wc = new sys.io.Process("wc", ["-l", logPath]);
                    var wcOut = new StringBuf();
                    var wcBuf = haxe.io.Bytes.alloc(256);
                    try {
                        var len = wc.stdout.readBytes(wcBuf, 0, wcBuf.length);
                        wcOut.add(wcBuf.sub(0, len).toString());
                    } catch (e:Dynamic) {}
                    try { wc.exitCode(); } catch (e:Dynamic) {}
                    wc.close();
                    beforeCount = (Std.parseInt(StringTools.trim(wcOut.toString())) != null) ? Std.parseInt(StringTools.trim(wcOut.toString())) : 0;
                } catch (e:Dynamic) {}
            }
            ...
            // Wait for Factorio to process
            Sys.sleep(1);

            // Get new lines via tail
            var output = "";
            if (sys.FileSystem.exists(logPath)) {
                try {
                    var wc2 = new sys.io.Process("wc", ["-l", logPath]);
                    var wcOut2 = new StringBuf();
                    var wcBuf2 = haxe.io.Bytes.alloc(256);
                    try {
                        var len = wc2.stdout.readBytes(wcBuf2, 0, wcBuf2.length);
                        wcOut2.add(wcBuf2.sub(0, len).toString());
                    } catch (e:Dynamic) {}
                    try { wc2.exitCode(); } catch (e:Dynamic) {}
                    wc2.close();

                    var afterCount = (Std.parseInt(StringTools.trim(wcOut2.toString())) != null) ? Std.parseInt(StringTools.trim(wcOut2.toString())) : 0;
                    var newLineCount = afterCount - beforeCount;

                    if (newLineCount > 0) {
                        // Cap at 200 lines to avoid huge responses
                        var tailCount = Math.min(newLineCount, 200);
                        var tail = new sys.io.Process("tail", ["-n", Std.string(tailCount), logPath]);
                        tail.stdin.close();
                        var tailBuf = haxe.io.Bytes.alloc(4096);
                        try {
                            while (true) {
                                var len = tail.stdout.readBytes(tailBuf, 0, tailBuf.length);
                                if (len == 0) break;
                                output += tailBuf.sub(0, len).toString();
                            }
                        } catch (e:haxe.io.Eof) {}
                        try { tail.exitCode(); } catch (e:Dynamic) {}
                        tail.close();
                    }
                } catch (e:Dynamic) {}
            }

            return if (StringTools.trim(output).length > 0) StringTools.trim(output) else "Command sent: " + command;
        } catch (e:Dynamic) {
            return "Error: " + e;
        }
    }
