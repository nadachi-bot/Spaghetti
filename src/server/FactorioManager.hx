...
class FactorioManager {
    ...
    public function syncMods(instance:ServerInstance):Void {
        // Allow syncing even when mods list is empty — the first start may
        // have an empty list, and --sync-mods will populate it from the save.
        ...
        try {
            ...
            // Read the generated mod-list.json and populate the instance's mod list.
            // This also prevents removeDisabledMods from deleting all mods when
    ...
    /**
     * Read the mod-list.json file generated by Factorio after --sync-mods.
     ...
     */
    public function readModListJson(modsDir:String):Array<ModInfo> {
        var filePath = modsDir + "/mod-list.json";
        ...
        try {
            ...
            // mod-list.json structure: { "mods": [ { "name": "..", "enabled": true, "version": ".." }, ... ] }
            var modsArr = cast(getFallback(data, "mods", []), Array<Dynamic>);
            if (modsArr == null) return [];

            for (raw in modsArr) {
                var name = cast(getFallback(raw, "name", ""), String);
                var version = cast(getFallback(raw, "version", ""), String);
            ...
            haxe.Log.trace("Failed to parse mod-list.json: " + e);
        }
